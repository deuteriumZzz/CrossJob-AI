...
- [Что нового](#что-нового-2026-09-04)
- [Площадки](#площадки)
...
## Что нового (2026-09-04)

- **Аудит резюме — наглядный результат вместо стены текста** — кнопка "Проверить" в дашборде теперь показывает процент соответствия с цветным индикатором (зелёный/жёлтый/красный), бэйдж "пройдёт/не пройдёт ATS", бэйдж решения нанимающего менеджера (да/возможно/нет), отсутствующие ключевые слова чипами и тревожные сигналы/сильные/слабые разделы списками — вместо трёх абзацев сырого текста LLM. Технически первые два шага аудита теперь возвращают структурированный JSON, а не прозу.
- **Логирование воронки поиска** — на каждом прогоне в лог пишется сводка по всем источникам: сколько найдено / уже видели / отсеялось по оценке / откликнулись / только просмотр (dry-run). Помогает понять, почему дневной квота не набирается — по какому критерию теряются вакансии на каждой площадке (нет новых, низкие оценки, превышен лимит).
- **Защита от краша на мёртвой вакансии** — если во время детальной загрузки одной вакансии площадка вернула 404, бот пропускает эту вакансию (логирует + продолжает), а не падает целиком. Распределено по всем источникам.
- **Структурные правила против "AI-текста"** — сверх текущих чёрных списков слов-шаблонов теперь есть ещё структурные проверки: эм-даш как соединитель, паттерны "не только X, но и Y", общие концовки, принудительные трёхсписки. Подключены в каждое сопроводительное письмо, ответ работодателю в чате HH и — теперь также — в генерацию резюме с нуля и в переписывание раздела опыта из аудита резюме (кнопка "Проверить" в дашборде).
- **Файловое логирование по умолчанию** — `LOG_TO_FILE = True` в `config.py`, дашборд читает логи из файла (`log/app.log`), поэтому вкладка "Логи" теперь всегда имеет что показывать (раньше была пуста, если логирование в файл не включили вручную).
- Проверка статуса отклика на HeadHunter теперь дополнительно обрабатывает отдельную страницу формы (`/applicant/vacancy_response`) — раньше распознавалась только модалка на странице вакансии.
