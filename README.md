...
| Раздел | Что там |
|---|---|
| **Главная** | «Что сделать сейчас» (черновики на отправку, новые ответы, интервью, контакты HR, площадки на паузе — по клику ведёт куда нужно), статистика, площадки |
...
| **Общение** | Входящие (ответы + «Ждут вашего решения») · База компаний (всё собранное с источником, фильтр «из моего файла / Telegram / вакансии», импорт файла) · Рассылка (база → письма → отправка через Gmail + статистика) · Telegram-парсер (каналы в реальном времени, контакты HR — в базу) |
| **Аналитика** | воронка до оффера, спрос на навыки, зарплаты, офферы, чего не хватает |
...
- **Главная** — «Результат за 7 дней» (ответы, интервью и отклик в % по каждому источнику, включая рассылку) и «Готовность» (что не подключено и куда нажать);
