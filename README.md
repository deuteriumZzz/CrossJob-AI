...
- [Что нового](#что-нового-2026-08-27)
- [Площадки](#площадки)
...
- ✍️ **Пишет** персональное сопроводительное письмо под каждую вакансию с помощью LLM, на основе вашего PDF-резюме (`data_folder/resume.pdf`) — само резюме не переписывается.
...
- Позиции/локации теперь можно переопределить для одной конкретной площадки (включая Telegram), не трогая общий список — пусто у площадки означает "используй общий" (см. `effective_list` в [GUIDE.md](docs/GUIDE.md)).
...
- Дашборд: кнопка "🔍 Аудит резюме" — LLM-цепочка в 3 шага (оценка соответствия 0-100 + ATS-ключевики + тревожные сигналы → отдельная проверка ATS-фильтром и «менеджером по найму» → переписанный раздел опыта по формуле Google XYZ, только на реальных данных из резюме, без выдумывания стажа). Сопроводительные письма (сайт-версия для HH/GetMatch/Wellfound/Himalayas и расширенная PDF-версия) переписаны под более конкретную структуру: явный пробел в требованиях называется прямо вместо замалчивания, конкретная причина интереса именно к этой компании.
- Дашборд: "Резюме и сопроводительное письмо" стало отдельным пунктом бокового меню (было спрятано в "Настройки") — генерация документов (резюме/резюме под вакансию/письмо) и аудит визуально разделены на разные карточки, а не смешаны в одном ряду кнопок. Кнопки "Запустить"/"Пауза" демона объединены в одну (иконка и подпись меняются по состоянию: Запустить → Пауза → Возобновить), вместо трёх отдельных рядом. Карточка площадки теперь показывает "Откликов сегодня X/Y" даже в режиме "только поиск" — раньше счётчик пропадал вместе с лимитом.
...
| Himalayas      | 🟡 experimental — поиск и автоотклик реализованы (`undetected-chromedriver`, как LinkedIn), но НЕ сверены на живом аккаунте: анонимный доступ к /jobs и /companies/... блокируется анти-бот интерстишлом сайта, см. [GUIDE.md](docs/GUIDE.md) |
| careerspace.app| план, низкий приоритет (проверено: `/jobs` — маленькая персонализированная лента без поиска по должности и без пагинации; куда ведёт "Ссылка на отклик" — не подтверждено) |
...
```bash
python main.py --auto headhunter   # или superjob / geekjob / rabota_ru / telegram / getmatch / linkedin / habr_career / wellfound / himalayas
...
Сборка в готовый `.app`/`.exe` — в [GUIDE.md](docs/GUIDE.md#сборка-в-exe-macos-и-windows).

## Лимиты и анти-бан

Дневной лимит и лимит за один прогон — это **наши собственные** настройки (пацинг, чтобы не выглядеть ботом), не официальные цифры площадок — они либо нигде публично не задокументированы, либо неприменимы (у geekjob/rabota.ru/GetMatch нет официального API вообще, только скрейпинг; у Telegram — это личный аккаунт, а не API-интеграция с лимитами площадки).

Значения ниже — дефолты из `config.py`, меняются на ходу в дашборде (**Настройки → Лимиты откликов**) без перезапуска и правки кода.
...
| geekjob.ru | Своего дневного лимита нет — `job_max_applications` за прогон | 5 | Нет официального API — кулдаун 24ч при признаках капчи/бана |
| rabota.ru | Своего дневного лимита нет — `job_max_applications` за прогон | 5 | Нет официального API — тот же кулдаун |
| GetMatch | Своего дневного лимита нет — `job_max_applications` за прогон | 5 | Нет официального API — тот же кулдаун |
...
| Wellfound | Общий `DAILY_APPLICATION_LIMIT` (если не задан свой в блоке `wellfound:`) | 5 | Официального API нет — кулдаун 24ч при признаках капчи/бана |
| Himalayas | Общий `DAILY_APPLICATION_LIMIT` (если не задан свой в блоке `himalayas:`) | 5 | Официального API нет — тот же кулдаун; сайт также показывает анти-бот интерстишл при подозрительном трафике |
...
- **`data_folder/secrets.yaml`** — ключ LLM (`llm_api_key`, по умолчанию для OpenAI) и, для каждой площадки, свои ключи (HeadHunter секретов не требует — вход браузерный; SuperJob — https://api.superjob.ru/register/, Telegram — https://my.telegram.org/apps). Провайдер LLM не обязан быть OpenAI — поддерживаются 12 провайдеров, каждый со своим ключом в `llm_api_keys:` (переключение и ввод ключей — в дашборде, без правки `config.py`). **Рекомендация по умолчанию — Groq, модель `openai/gpt-oss-120b`** (👑 в выборе модели): бесплатно и достаточно для оценки вакансий/писем.
- **`data_folder/work_preferences.yaml`** — позиции, локации, чёрные списки компаний/названий/локаций, фильтры по опыту/типу занятости/дате, и опциональные блоки на площадку (`auto_apply`, `resume_id`, `schedule_enabled`/`interval_hours`) и `telegram:` (`channels`, `auto_message`, `daily_message_limit`, `max_post_age_days`). Любую площадку можно точечно переопределить своими `positions`/`locations` внутри её блока — пусто там означает "используй общий список сверху".
