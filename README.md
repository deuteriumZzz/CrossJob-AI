...
## Что нового (2026-08-26)

- HeadHunter: один Chrome-процесс на весь прогон (поиск + карточки вакансий + отклики) вместо открытия/закрытия браузера на каждый шаг — быстрее и меньше зависаний. То же для GetMatch.
- HeadHunter: автоответ на тест/анкету вакансии перед откликом (эвристика, без AI по умолчанию).
- HeadHunter: опциональный бесплатный "бамп" резюме перед прогоном (`auto_bump_resume`) — аналог платного "поднять резюме в поиске".
- HeadHunter: ручные действия из дашборда — блокировка работодателя, клонирование резюме, создание черновика резюме, отмена зависших/отклонённых откликов (`auto_cleanup_negotiations`).
- SuperJob и zarplata.ru: отдельное расписание проверки ответов в чате (`check_sj_replies`/`check_zp_replies`), тот же принцип, что раньше был только у HH — раньше не входило в цикл демона вовсе.
- Общий таймаут на загрузку страницы (45с) и скрипты (30с) для всех браузерных площадок — раньше зависший Selenium мог остановить весь прогон без единой ошибки в логе.
- Дашборд: карточка площадки сама переключает участие в расписании демона галочкой (вместо отдельной кнопки "Запустить выбранные"); убрано мерцание списка площадок/истории при опросе статуса; виден статус готовности резюме (PDF в `data_folder`) для HH/LinkedIn.
...
Значения ниже — дефолты из `config.py`. Их можно менять на ходу, без перезапуска и правки кода, в дашборде: вкладка **Настройки → Лимиты откликов** — как общие (все площадки сразу), так и переопределить для одной конкретной площадки (или вручную — блок `limits:`, либо `job_max_applications`/`daily_application_limit` внутри блока нужной площадки в `work_preferences.yaml`). Чем выше значения — тем больше шанс, что площадка распознает автоматизацию; дашборд показывает этот риск прямо рядом с настройкой, но решение — за пользователем.
...
Дашборд умеет и первоначальную настройку с нуля (свой веб-мастер —
не нужен `data_folder/` заранее), и точечные действия без перезапуска:
редактировать должности/локации/чёрные списки (вкладка Настройки →
Поиск) — включая кнопку "Сгенерировать должности из резюме" (LLM сам
предложит 2-4 подходящие должности по PDF-резюме, без правки
work_preferences.yaml вручную), пересобрать `plain_text_resume.yaml`
...
(OpenAI/Groq/Gemini/DeepSeek/NVIDIA NIM/OpenRouter/Ollama) со своим API-ключом на каждого —
