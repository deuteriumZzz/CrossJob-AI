...
| Хабр Карьера   | план (официальный API непригоден для этого проекта — доступ по ручному одобрению Хабра, не для личных ботов; нужна полноценная браузерная интеграция как у HH: вход через SSO Habr, поиск и отклик — скрейпинг, подтверждено вживую 2026-08-28 — кнопка "Откликнуться" на странице вакансии обычный JS-`<button>` без ссылки, отдельной страницы входа `/users/login` нет) |
...
| Хабр Карьера | 🟢 Высокий (для IT) | Одна из самых понятных площадок именно для IT-поиска, зрелая экосистема (рейтинг компаний, зарплаты, журнал) — но к боту пока не подключена: нужна полноценная браузерная интеграция как у HH (см. таблицу выше), не просто API |
