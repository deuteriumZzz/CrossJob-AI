...
Значения ниже — дефолты из `config.py`. Их можно менять на ходу, без перезапуска и правки кода, в дашборде: вкладка **Настройки → Лимиты откликов** (или вручную — блок `limits:` в `work_preferences.yaml`). Чем выше значения — тем больше шанс, что площадка распознает автоматизацию; поднимайте постепенно.
