...
- 🛡️ **Защищается от бана**: случайные паузы между откликами, суточные лимиты на площадку и опциональный общий лимит на все площадки сразу, `apply_once_at_company`. Одна и та же вакансия, висящая на нескольких площадках, получает только один отклик (сверка по компании и должности).
- 📬 **Следит, что было после отклика**: читает статусы откликов hh (просмотрен / приглашение / отказ), ведёт этап по каждой заявке (ответили → интервью → тестовое → оффер / отказ) и собирает все ответы в «Входящие». На ответ HR в Telegram готовит черновик ответа, если HR молчит — предлагает одно напоминание; утром присылает сводку.
- 👤 **Находит контакты HR**: вкладка «Контакты» — компании и люди, которым можно написать напрямую (email, Telegram, LinkedIn), у каждого контакта источник и дата. Посты Telegram-каналов разбираются целиком (компания, должность, зарплата, все контакты), «Досье» ищет контакты на сайте компании и через Hunter. Только то, что опубликовано, — без угаданных адресов.
- ✉️ **Пишет HR напрямую**: первое сообщение по правилу «три требования вакансии → три ваших примера», письма с вашей почты Gmail (пароль приложения) с напоминанием в той же ветке. Ничего не уходит без подтверждения — «Отправить» во «Входящих» или «отправить &lt;код&gt;» в Telegram-боте.
- 🌍 **Прямой поиск**: вакансии прямо с сайтов компаний (Greenhouse, Lever, Ashby, Workable — система найма определяется по сайту), We Work Remotely и Hacker News «Who is hiring»; форма отклика Greenhouse/Lever заполняется кнопкой, отправку (там капча) делаете вы.
- 📊 **Аналитика рынка**: воронка до оффера, спрос на навыки против вашего резюме, медианы зарплат по валютам, сравнение офферов; удалёнка «только США» отсекается.
- 🎯 **Помогает с интервью**: справка к интервью (вероятные вопросы, как говорить о слабых местах), тренажёр с разбором ответов, событие `.ics` для календаря.
- 🖥️ **Дашборд**: нативное окно с обзором площадок, историей, входящими, контактами, аналитикой и настройками — без правки YAML руками.
...
| Telegram-каналы| ⚡ мгновенные уведомления: постоянное подключение, новый пост с ключевым словом сразу приходит в бот уведомлений с контактом HR и кнопками «👋 Здравствуйте», «👋 + 📎 резюме» (PDF из `data_folder/telegram/`), «✍️ сопроводительное под вакансию» (LLM, письма сохраняются в `data_folder/telegram/letters/`) — без LLM-фильтра и расписания; ✅ поиск по расписанию + полный разбор поста (компания, должность, зарплата, все контакты → вкладка «Контакты») + опциональные автосообщения; ответы HR разбираются (интерес/вопрос/отказ) и получают черновик ответа на подтверждение (личный аккаунт через официальный API Telegram, см. [GUIDE.md](docs/GUIDE.md)) |
...
| Прямой поиск   | ✅ сайты компаний на Greenhouse/Lever/Ashby/Workable (открытые API, без входа), We Work Remotely (RSS), HN «Who is hiring»; автоотклика нет — «нужен ручной отклик» + кнопка «Заполнить форму» для Greenhouse/Lever (сверено на живых формах 2026-09-24). RemoteOK/Remotive не подключены: отклик у первого платный, у второго страница за Cloudflare |
| Djinni         | ✅ поиск (JSON-LD выдачи djinni.co обычными HTTP-запросами, без браузера — сверено на живом запросе 2026-09-25: категория технологии + полнотекстовый поиск, 3 страницы на должность) · 🟡 автоотклик — сообщение рекрутеру с сопроводительным письмом через браузер с вашим входом (вход вручную один раз); сверен на живом аккаунте 2026-09-25: вакансии, куда Djinni не пустит (стаж, страна, английский, зарплата), отсеиваются при поиске и проверкой страницы до письма, с причиной в «Вакансиях»; по умолчанию «Только ищет» · 🔝 поднятие профиля («Bump My Profile») раз в 7 дней — переключатель в окне площадки, как бамп резюме на HH |
...
```bash
python main.py --auto headhunter   # или geekjob / getmatch / telegram / linkedin / habr_career / wellfound / himalayas / djinni
...
Пять разделов вместо россыпи вкладок, каждый под свою задачу:

| Раздел | Что там |
|---|---|
| **Главная** | мастер настройки по шагам (пока что-то не подключено), «Свои каналы» (карточки Telegram-парсера и отправки почты), «Что сделать сейчас» (черновики на отправку, новые ответы, интервью, контакты HR, площадки на паузе — по клику ведёт куда нужно), статистика, площадки |
| **Вакансии** | всё, что нашёл бот: статус, этап, балл, письмо; у вакансии «Действия» — найти HR, подготовка к интервью, тренажёр, календарь, заполнить форму |
| **Общение** | Входящие — что ответили работодатели (площадки, Telegram, письма), по умолчанию «Главное»: интервью и вопросы HR, отказы отдельно · Telegram-парсер (как работает, подключение, каналы, диалоги) |
| **Компании** | База — таблица всех компаний и HR: статус («не писали / написали / ответили / возврат / не писать»), источник, последнее действие, сортировка, действия над выбранными, выгрузка в Excel, ежедневная резервная копия · Рассылки — письма через Gmail + статистика и напоминания, кнопки «Отправить все / по одному» прямо в CrossJob-боте |
| **Аналитика** | результат за 7 дней по источникам, воронка до оффера, спрос на навыки, зарплаты, офферы, чего не хватает |
| **Настройки** | поиск, площадки, лимиты, LLM, контакты и письма, уведомления · Резюме · Логи |

Всё в одном окне, без ручной правки YAML.
...
- **Входящие** — ответы hh и HR из Telegram одним списком, этап по каждой заявке и блок «Ждут вашего решения» с черновиками ответов, писем и напоминаний;
- **Контакты** — компании и HR с источником каждого контакта, статус (не писали / черновик / написали / ответили), кнопки «Черновик сообщения/письма» и «Досье», отслеживаемые компании для «Прямого поиска»;
- **История → «Действия»** — подготовка к интервью, тренажёр, событие в календарь, заполнение формы отклика;
- **Аналитика** — воронка до оффера, спрос на навыки, зарплаты на рынке, сравнение офферов;
- **База компаний → «📥 Загрузить свой файл»** — свой список компаний и HR (Excel/CSV/TXT/PDF/Word, например ответ Claude Research): таблица читается по заголовкам, свободный текст раскладывает LLM (адрес берётся, только если он дословно есть в файле), адреса проверяются по MX-записи домена. Всё ложится в ту же базу, что Telegram и вакансии; фильтр по источнику показывает, что пришло из файла;
- **Telegram-парсер** — постоянное подключение к каналам (не опрос раз в N часов): пост с вашими ключевыми словами за секунды приходит в бота с кнопками «Здравствуйте» / «+ резюме» / «сопроводительное под вакансию», контакты HR из поста сразу ложатся в «Базу компаний». Новые каналы подхватываются на ходу (в закрытые для аккаунта парсер вступает сам), на вкладке видно: сколько каналов слушает, сколько вакансий нашёл, сколько контактов собрал;
- **Аналитика** — «Результат за 7 дней» (ответы, интервью и отклик в % по каждому источнику, включая рассылку); на Главной — «Готовность» (что не подключено и куда нажать). Проверки ответов (hh, Telegram, почта, команды бота) включены сами и работают, как только подключён их канал;
- **Рассылка** — три шага: ① База (сколько компаний с email, которым ещё не писали) → ② Письма (отдельное письмо каждой компании по промту: кто я → почему вы → 2–3 достижения → звонок, 150–180 слов, тема «[Должность] Application — Имя Фамилия»; можно прочитать, поправить или убрать) → ③ Отправка одной кнопкой через Gmail по одному с паузой 1–2 мин и дневным лимитом, резюме во вложении. Статистика: отправлено / ответили / ошибка (с причиной) / возврат. Кто молчит 7+ дней — готовое короткое напоминание в той же ветке письма, отправка одной кнопкой;
- **Настройки → «Контакты и письма»** — почта Gmail (с проверкой «письмо себе»), ключ Hunter, утренняя сводка, дни до напоминания, фильтр удалёнки по региону.
...
- **Контакты и письма** (удобнее в дашборде: Настройки → «Контакты и письма»): в `secrets.yaml` — `email: {address, app_password}` (пароль приложения Google, не основной пароль) и необязательный `hunter_api_key`; в `work_preferences.yaml` — блок `direct:` («Прямой поиск»: `schedule_enabled`, `wwr`, `hn`, `email_outreach`, `email_daily_limit`, `follow_up_days`), `telegram.follow_up_days`, `digest: {enabled, hour}`, `excluded_remote_regions` (по умолчанию `[us_only]`) и расписания проверок `check_hh_replies`, `check_telegram_replies`, `check_email_replies`.
- **Логи** — `log/app.log` (ротация 10 МБ, хранение неделю); раньше файл стирался при каждом запуске, теперь история сохраняется между перезапусками, а тесты в него не пишут.
