...
# Apply, чем остальные источники.
LINKEDIN_DAILY_APPLICATION_LIMIT = 8

# Сколько дней хранить текст сгенерированного сопроводительного письма
# в applied_log.json (для контроля качества LLM — заметить, если она
# пишет чушь) — после истечения срока очищается только само письмо
# (см. AppliedLog.purge_old_cover_letters), остальная запись остаётся
