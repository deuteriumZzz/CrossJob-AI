...
class WellfoundClient:
    ...
    def apply(
        ...
        анонимно подтверждено только, что кнопка "Apply Now" открывает
        модалку "Your Application", которая для незалогиненного
        пользователя просит завести аккаунт (имя, email, СВОЙ пароль).
        Если модалка после клика всё ещё содержит поле пароля — сессия
        не аутентифицирована (WellfoundSession.ensure_logged_in() не
        был пройден вручную, либо профиль сброшен), и мы НИКОГДА не
        заполняем и не отправляем эту форму сами (это было бы
        созданием аккаунта за пользователя) — возвращаем False. Иначе
        пробуем как HH.answer_full_page_questionnaire: обобщённо
        отвечаем на видимые текстовые/select/radio/checkbox поля и
        жмём кнопку с текстом submit/apply/send application; если
        такой кнопки не нашлось — форма не подтверждена, тоже False."""
        ...
        try:
            ...
            apply_buttons = driver.find_elements(
                By.CSS_SELECTOR, '[data-test="JobApplicationApplyButton"]'
            )
