...
def _daily_limit(parameters: dict, source: Optional[str] = None) -> int:
    ...
    внутри блока конкретной площадки (headhunter:/geekjob:/...,
...
# (и сама возможность увидеть новые ответы Telegram) никогда не
...
@click.option(
    ...
    help=(
        ...
        "'selected:hh,geekjob' for a comma-separated subset of sources."
    ),
)
@click.option(
    "--daemon",
...
def main(auto: Optional[str], daemon: bool):
    ...
    try:
        ...
        # чтобы каждый источник (HH/geekjob/Telegram) искал роли,
        # которые реально подходят этому резюме.
        resume_pdf_path = data_folder / RESUME_PDF
